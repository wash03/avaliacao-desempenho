...
( function ($) {
    ...
    $.fn.slideform = function( userOptions, args ) {
        ...
        var $options = {
            ...
            validate: null,
            submit: null,
            slides: $form.find('.slideform-slide'),
        }
        ...
        */
        ...
        function init() {
            ...
            $form.find('.slideform-slide').each( function (k, v) {
                ...
                if ( k < $options.slides.length - 1 ) {
                    $(this).find('.slideform-group').append('<button class="slideform-btn slideform-btn-prev">' + $options.prevButtonText + '</button>');
                    $(this).find('.slideform-group').append('<button class="slideform-btn slideform-btn-next">' + $options.nextButtonText + '</button>');
                } else{
                    $(this).find('.slideform-group').append('<button class="slideform-btn slideform-btn-prev">' + $options.prevButtonText + '</button>');
                    $(this).find('.slideform-group').append('<button class="slideform-btn" onclick="window.print();"><i class="icon-check"></i>Imprimir/Salvar</button>');
        ...
        $form.goTo = function( slide ) {
            var translation = slide * ( 100 / $options.slides.length );
            var percentage = 0;

            if ( slide - 1 <= 0 ) {
                percentage = 0;
            } else if ( slide == ( $options.slides.length - 1 )) {
                percentage = 100;
            } else {
                percentage = (slide - 1) * ( 100 / ($options.slides.length - 2) );
            }
            ...
            $options.slides.filter('.active').animate({ scrollTop: 0}, 'fast' );

            // Let's set the active class.
            $options.slides.eq( slide ).addClass('active').siblings().removeClass('active');
            ...
            $('html, body').animate({
                scrollTop: $(".avaliacao").offset().top
            }, 200);
            setTimeout(function(){
        ...
        $form.goForward = function() {
            ...
            if ( $current == ( $options.slides.length - 1 )) return;
        ...
        */
        ...
        function updateButtonsStatus( slideNumber ) {
            $form.find('.slideform-btn-next').prop('disabled', ( slideNumber == $options.slides.length - 1) );
        ...
        function validateSlideInputs ( slideNumber ) {
            ...
            $options.slides.eq( slideNumber ).find('input, textarea, select').each( function () {
